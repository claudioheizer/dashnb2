...
server <- function(input, output) {
  ...
  output$tabelaSemanas <- renderDataTable({
    ...
    for (i in 1:nrow(dadosFiltrados1)) {
      ...
      semana_ent <- seq(sem_ini_hosp_ent, sem_fim_hosp_ent)
      semana_pront <- seq(sem_ini_hosp_pront, sem_fim_hosp_pront)
    ...
    DT::datatable(
      ...
      options = list(
        ...
        initComplete = JS(
          ...
          "    var instrumento = data[2];",
          "    var semanas_inicio = data[semanas_inicio];",
          "    var semanas_fim = data[semanas_fim];",
          "    var semana_ent = data[semana_ent];",
          "    var semana_pront = data[semana_pront];",
          "    var cellIndex = 3; // Índice da primeira célula a ser colorida",
          "    var colIndex = 7; // Índice da coluna de início das semanas",
          "    var cell;",
          
          "    for (var i = 0; i < semana_ent.length; i++) {",
          "      if (instrumento === 'Entrevistas' && semana_ent[i] >= semanas_inicio && semana_ent[i] <= semanas_fim) {",
          "        cell = $(row).find('td:eq(' + (colIndex + (semana_ent[i] - semanas_inicio)) + ')');",
          "        cell.css('background-color', '#006400');",
          "        cell.css('color', '#FFFFFF');",
          "      }",
          "    }",
          
          "    for (var j = 0; j < semana_pront.length; j++) {",
          "      if (instrumento === 'Prontuários' && semana_pront[j] >= semanas_inicio && semana_pront[j] <= semanas_fim) {",
          "        cell = $(row).find('td:eq(' + (colIndex + (semana_pront[j] - semanas_inicio)) + ')');",
